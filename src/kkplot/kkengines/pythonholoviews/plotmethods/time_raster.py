...
def kkplot_pythonholoviews_time_raster( self, _id, _graph, _axes_index, _columns, _auxialiary_columns, **_kwargs) :

    agg_method = _graph.get_property( 'agg', 'mean')

    axes = '%s["%s"]' % ( _kwargs['axes'], _axes_index)
    dataframe = _kwargs['dataframe']
    method_call = 'kkplot_plot_time_raster_%s( "%s", _dataframe=%s)' % ( self._canonicalize_name( _id), _id, dataframe)
    ...
    w( 0, 'def kkplot_plot_time_raster_%s( _id, _dataframe) :' % ( self._canonicalize_name( _id)))
    #w( 1, '_axes.set_gid( _id)')
    w( 1, 'import holoviews as hv')
    ...
    if len(xycolumns) == 3 :    
        ...
        w( 2+indentation, 'return hv.Raster( raster)')
