...
## example call
## python kkplot.py -E holoviews examples/config.yml
##
...
from kkplot.kkengines.pythonholoviews.plotmethods import *
...
class kkplot_engine_holoviews( kkplot_engine) :
    def  __init__( self, _conf=None, _dviplot=None) :
        super( kkplot_engine_holoviews, self).__init__( "holoviews", _conf, _dviplot)

        self.W = None

        kkplot_plotmethods = dict( \
            time_raster=kkplot_pythonholoviews_time_raster, \
            none=None)
    ...
    def  new( self, _conf, _dviplot) :
        engineholoviews = kkplot_engine_holoviews( _conf, _dviplot)
        engineholoviews.W = kkplot_writer( _stream=None, _mode='python')
        return  engineholoviews
    ...
    def generate_layout( self) :
        self.W.iappendnl( 1, '')
        self.W.iappendnl( 1, 'renderer = hv.renderer("bokeh")')
        ...
        for graph, plot in self.dviplot :
            ...
            self.W.iappendnl( 1, 'kkaxes["%s"] = renderer.get_plot( graphresults["%s"]).state' %(ax_index, '.'+graph.graphid+'.result_'))
            if define_layout:
    ...
    def generate_plots_writefigure( self) :
        ...
        #self.W.iappendnl( 1, 'renderer = hv.renderer("bokeh")')
        #self.W.iappendnl( 1, 'plot = None')
        #self.W.iappendnl( 1, 'for k,v in graphresults.items():')
        #self.W.iappendnl( 2, 'if plot == None :')
        #self.W.iappendnl( 3, 'plot = renderer.get_plot( v).state')
        #self.W.iappendnl( 2, 'else:')
        #self.W.iappendnl( 3, 'plot += renderer.get_plot( v).state')
        #tl_top = 0.95
    ...
    def generate_preamble( self, _dviplot) :
        ...
        self.W.appendnl( 'import holoviews as hv')
        self.W.appendnl( 'from bokeh.io import output_file, save, show')
        ...
        self.W.appendnl( 'from bokeh.models import Range1d, Div')
        self.W.appendnl( 'hv.extension("bokeh")')
        self.W.appendnl( '')
...
__kkplot_engine_holoviews_factory = kkplot_engine_holoviews()
